...
% TODO: TO CHANGE THE util Path
addpath('MetricEvaluation\utils');
scores = calc_scores2(input_dir,GT_dir,shave_width,verbose);
% Saving
%save(strcat(test_name,'.mat'),'scores');

%% Printing results
perceptual_score = (mean([scores.NIQE]) + (10 - mean([scores.Ma]))) / 2;
res=[perceptual_score,mean([scores.Ma]),mean([scores.NIQE]),mean([scores.MSE]),sqrt(mean([scores.MSE])),mean([scores.PSNR]),mean([scores.SSIM]),mean([scores.BIQME]),mean([scores.FADE]),mean([scores.AG]), mean([scores.IE]), mean([scores.VAR])];
